 ...
 * @returns {{ isCointegrated: boolean, adfStat: number, meanReversionRate: number, zScore: number }}
 */
function testCointegration(prices1, prices2, beta) {
  ...
  // ADF-like test for stationarity
  const spreadDiffs = [];
  for (let i = 1; i < spreads.length; i++) {
    spreadDiffs.push(spreads[i] - spreads[i-1]);
  }
...
/**
 * Quick pair fitness check - returns correlation, cointegration, z-score
 * @param {number[]} prices1 - First price series (e.g., 30 days)
 * @param {number[]} prices2 - Second price series
 * @returns {{ correlation: number, beta: number, isCointegrated: boolean, zScore: number, meanReversionRate: number }}
 */
function checkPairFitness(prices1, prices2) {
  const { correlation, beta } = calculateCorrelation(prices1, prices2);
  const { isCointegrated, zScore, meanReversionRate } = testCointegration(prices1, prices2, beta);
