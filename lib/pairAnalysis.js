...
function testCointegration(prices1, prices2, beta) {
  ...
  // Half-life calculation using AR(1) on spread levels (canonical method)
  // Uses the unified calculateHalfLifeAR1 function for consistency
  const halfLife = calculateHalfLifeAR1(spreads) ?? Infinity;

  // Cointegration check: autocorrCoeff < 0.3 indicates low serial correlation (more reliable)
  const isCointegrated = adfStat < -2.5 || (meanReversionRate > 0.5 && Math.abs(autocorrCoeff) < 0.3);
