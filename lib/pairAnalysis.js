...
async function analyzePair(config) {
  ...
  try {
    if (fullPrices1?.length >= 30 && fullPrices2?.length >= 30) {
      ...
      // Calculate 30-day daily z-scores for volatility analysis
      const prices1_30d = fullPrices1.slice(-30);
      const prices2_30d = fullPrices2.slice(-30);
      const zScoreLen = Math.min(prices1_30d.length, prices2_30d.length);
      let recentZScores = [];
      
      if (zScoreLen >= 15) {
        const spreads30d = [];
        for (let i = 0; i < zScoreLen; i++) {
          spreads30d.push(Math.log(prices1_30d[i]) - beta30d * Math.log(prices2_30d[i]));
        }
        const meanSpread = spreads30d.reduce((a, b) => a + b, 0) / spreads30d.length;
        const stdSpread = Math.sqrt(
          spreads30d.reduce((sum, s) => sum + Math.pow(s - meanSpread, 2), 0) / spreads30d.length
        );
        if (stdSpread > 0) {
          recentZScores = spreads30d.map(s => (s - meanSpread) / stdSpread);
        }
      }
      
      const regime = detectRegime(zScore, entryThreshold, recentZScores, hurst.hurst);
