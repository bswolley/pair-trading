...
/**
 * Calculate half-life using AR(1) regression on spread levels
 * This is the canonical method - use this everywhere for consistency
 * 
 * spread[t] = α + φ * spread[t-1] + ε[t]
 * half_life = -ln(2) / ln(φ) where 0 < φ < 1
 * 
 * @param {number[]} spreads - Array of spread values (log(P1) - β*log(P2))
 * @returns {number|null} Half-life in days, or null if calculation fails
 */
function calculateHalfLifeAR1(spreads) {
  if (!spreads || spreads.length < 10) return null;
...
async function analyzeTimeframe(symbol1, symbol2, days, fullOBV1, fullOBV2, obvTimeframes, sharedSdk = null) {
  ...
  // Correlation (on returns) & Beta (OLS on log prices)
  ...
  // Beta: OLS regression on log prices (proper hedge ratio for pairs trading)
  // log(P1) = α + β*log(P2) + ε
  ...
  // Half-life calculation using AR(1) on spread levels (canonical method)
  // Uses the unified calculateHalfLifeAR1 function for consistency across the system
  // spread[t] = α + φ * spread[t-1] + ε[t]
  // half_life = -ln(2) / ln(φ) where 0 < φ < 1
  let halfLife = calculateHalfLifeAR1(spreads);
...
async function analyzeHistoricalDivergences(symbol1, symbol2, sharedSdk = null, stdDevSpread = null, currentZScore = null) {
  ...
  try {
    ...
    // EPISODE-BASED COUNTING: One event per divergence episode
    // - New event only starts when Z crosses threshold AND no active episode exists
    // - Episode ends when Z reverts below target (0.5 for fixed, 50% of threshold for percentage)
    // - This prevents double-counting when Z oscillates around threshold
    const thresholds = [1.0, 1.5, 2.0, 2.5, 3.0];
    const profile = {};
    ...
    for (const threshold of thresholds) {
      // Fixed reversion (0.5 for all) - EPISODE-BASED
      let events = 0;
      let reverted = 0;
      const reversionTimes = [];
      let activeEpisode = null; // Only one active episode at a time

      // Percentage-based reversion (50% of threshold) - EPISODE-BASED
      let eventsPercent = 0;
      let revertedPercent = 0;
      const reversionTimesPercent = [];
      let activeEpisodePercent = null; // Only one active episode at a time
      const percentReversionThreshold = threshold * 0.5; // 50% of threshold

      // Track oscillations for informational purposes
      let crossings = 0;
      ...
      for (let i = 0; i < zScores.length; i++) {
        ...
        // Count all threshold crossings (for oscillation tracking)
        if (prevAbsZ < threshold && absZ >= threshold) {
          crossings++;
        }

        // FIXED REVERSION: Start new episode only if no active episode
        if (absZ >= threshold && activeEpisode === null) {
          // Start of a new divergence episode
          events++;
          activeEpisode = {
            startIndex: i,
        ...
        } else if (activeEpisode !== null && absZ >= threshold) {
          // Track peak Z during episode
          activeEpisode.peakZ = Math.max(activeEpisode.peakZ, absZ);
        }

        // PERCENTAGE-BASED: Start new episode only if no active episode
        if (absZ >= threshold && activeEpisodePercent === null) {
          eventsPercent++;
          activeEpisodePercent = {
            startIndex: i,
            startTime: currentTime,
            peakZ: absZ
          };
        } else if (activeEpisodePercent !== null && absZ >= threshold) {
          activeEpisodePercent.peakZ = Math.max(activeEpisodePercent.peakZ, absZ);
        }

        // Exit divergence: Fixed reversion (to < 0.5)
        if (activeEpisode !== null && absZ < fixedReversionThreshold) {
          reverted++;
          const timeToRevertHours = (currentTime - activeEpisode.startTime) / (60 * 60 * 1000);
          reversionTimes.push(timeToRevertHours);
          activeEpisode = null; // Episode closed
        }

        // Exit divergence: Percentage-based reversion (to < 50% of threshold)
        if (activeEpisodePercent !== null && absZ < percentReversionThreshold) {
          revertedPercent++;
          const timeToRevertHours = (currentTime - activeEpisodePercent.startTime) / (60 * 60 * 1000);
          reversionTimesPercent.push(timeToRevertHours);
          activeEpisodePercent = null; // Episode closed
        }
      }
      ...
      profile[threshold.toString()] = {
        events: completedEvents,  // Episode count (one per divergence)
        crossings: crossings,     // Total threshold crossings (for oscillation info)
        reverted: reverted,
        rate: rate.toFixed(1) + '%',
    ...
    // Find optimal entry threshold using percentage-based reversion (to 50% of threshold)
    // Option B: Highest threshold with >= 90% reversion rate and min 3 events
    // This is more realistic since traders typically exit at partial reversion, not full mean
    const MIN_ENTRY_THRESHOLD = 2.5; // Safety floor - never enter below this Z-score (raised from 2.0 based on performance data)
    let optimalEntry = MIN_ENTRY_THRESHOLD; // Default minimum
    for (let i = thresholds.length - 1; i >= 0; i--) {
      const threshold = thresholds[i];
      const stats = profilePercent[threshold.toString()];
    ...
    if (optimalEntry === MIN_ENTRY_THRESHOLD) {
      for (let i = thresholds.length - 1; i >= 0; i--) {
        const threshold = thresholds[i];
        const stats = profilePercent[threshold.toString()];
        const rate = parseFloat(stats.rate);
...
function calculateDualBeta(prices1, prices2, halfLife = 7) {
  ...
  // Helper: OLS regression on LOG PRICES (proper hedge ratio)
  // log(P1) = α + β*log(P2) + ε
  function olsOnLogPrices(p1, p2) {
    const n = p1.length;
    if (n < 5) return { beta: 0, r2: 0, stdErr: Infinity };

    // Convert to log prices
    const logP1 = p1.map(p => Math.log(p));
    const logP2 = p2.map(p => Math.log(p));
    ...
    const beta = varLogP2 > 0 ? covLogPrices / varLogP2 : 0;
    const alpha = meanLogP1 - beta * meanLogP2;

    // R² calculation on log prices
    let ssRes = 0, ssTot = 0;
    for (let i = 0; i < n; i++) {
      const predicted = alpha + beta * logP2[i];
      ssRes += (logP1[i] - predicted) ** 2;
      ssTot += (logP1[i] - meanLogP1) ** 2;
    }
    const r2 = ssTot > 0 ? Math.max(0, 1 - ssRes / ssTot) : 0;

    // Standard error of beta
    const mse = ssRes / (n - 2);
    const stdErr = varLogP2 > 0 ? Math.sqrt(mse / varLogP2) : Infinity;
  ...
  const structural = olsOnLogPrices(
 ...
 */
/**
 * Calculate correlation and hedge ratio (beta) between two price series
 * 
 * Correlation: Pearson correlation on returns (measures co-movement strength)
 * Beta: OLS regression on log prices (proper hedge ratio for pairs trading)
 *       log(P1) = α + β*log(P2) + ε
 * 
 * @param {number[]} prices1 - First price series (Y in regression)
 * @param {number[]} prices2 - Second price series (X in regression)
 * @returns {{ correlation: number, beta: number, alpha: number, rSquared: number }}
 */
function calculateCorrelation(prices1, prices2) {
  ...
  // === CORRELATION: Calculate on returns (measures co-movement) ===
  const returns = [];
  for (let i = 1; i < prices1.length; i++) {
    returns.push({
      asset1: (prices1[i] - prices1[i - 1]) / prices1[i - 1],
  ...
  // log(P1) = α + β*log(P2) + ε
  // This is the correct method for pairs trading spread calculation
  const n = prices1.length;
  const logP1 = prices1.map(p => Math.log(p));
  const logP2 = prices2.map(p => Math.log(p));
  ...
  // Calculate R² for the log price regression
  let ssRes = 0, ssTot = 0;
  for (let i = 0; i < n; i++) {
    const predicted = alpha + beta * logP2[i];
    ssRes += (logP1[i] - predicted) ** 2;
  ...
  return { correlation, beta, alpha, rSquared };
}

/**
 * Engle-Granger Critical Values (MacKinnon 1991, 2010)
 * For residual-based cointegration test with 2 variables, no trend
 * These are more stringent than standard ADF critical values
 * 
 * Values interpolated for common sample sizes
 */
const ENGLE_GRANGER_CRITICAL_VALUES = {
  // Sample size: { 1%: cv, 5%: cv, 10%: cv }
  25:  { '1%': -4.37, '5%': -3.59, '10%': -3.23 },
  50:  { '1%': -4.12, '5%': -3.46, '10%': -3.13 },
  100: { '1%': -3.96, '5%': -3.37, '10%': -3.07 },
  250: { '1%': -3.87, '5%': -3.31, '10%': -3.02 },
  500: { '1%': -3.83, '5%': -3.29, '10%': -3.00 }
};

/**
 * Get interpolated Engle-Granger critical value for given sample size
 * @param {number} n - Sample size
 * @param {string} level - Significance level ('1%', '5%', '10%')
 * @returns {number} Critical value
 */
function getEngleGrangerCriticalValue(n, level = '5%') {
  const sizes = [25, 50, 100, 250, 500];
  
  if (n <= 25) return ENGLE_GRANGER_CRITICAL_VALUES[25][level];
  if (n >= 500) return ENGLE_GRANGER_CRITICAL_VALUES[500][level];
  
  // Linear interpolation
  for (let i = 0; i < sizes.length - 1; i++) {
    if (n >= sizes[i] && n < sizes[i + 1]) {
      const n1 = sizes[i], n2 = sizes[i + 1];
      const cv1 = ENGLE_GRANGER_CRITICAL_VALUES[n1][level];
      const cv2 = ENGLE_GRANGER_CRITICAL_VALUES[n2][level];
      return cv1 + (cv2 - cv1) * (n - n1) / (n2 - n1);
    }
  }
  
  return ENGLE_GRANGER_CRITICAL_VALUES[100][level]; // Default fallback
}
 ...
 * @returns {{ alpha: number, beta: number, residuals: number[], rSquared: number, standardErrors: { alpha: number, beta: number } }}
 */
function olsRegression(y, x) {
  if (y.length !== x.length || y.length < 3) {
    throw new Error('OLS requires equal length arrays with at least 3 observations');
  }
  ...
  const seAlpha = Math.sqrt(mse * (1/n + meanX**2 / varX));
  const seBeta = Math.sqrt(mse / varX);
  ...
  return {
    ...
    residuals,
    rSquared,
    standardErrors: { alpha: seAlpha, beta: seBeta }
  };
}

/**
 * Proper Augmented Dickey-Fuller Test
 * 
 * Tests H0: series has unit root (non-stationary)
 * vs H1: series is stationary (mean-reverting)
 * 
 * Model: Δy_t = γ*y_{t-1} + Σ(c_i*Δy_{t-i}) + ε_t
 * 
 * @param {number[]} series - Time series to test
 * @param {number} maxLag - Maximum lag order (default: auto-selected)
 * @returns {{ adfStat: number, gamma: number, pValue: string, lags: number }}
 */
function augmentedDickeyFullerTest(series, maxLag = null) {
  const n = series.length;
  if (n < 10) {
    return { adfStat: 0, gamma: 0, pValue: 'insufficient_data', lags: 0 };
  }
  
  // Auto-select lag using rule of thumb: floor(12 * (n/100)^0.25) capped at reasonable value
  // For cointegration residuals, shorter lags are usually better
  if (maxLag === null) {
    maxLag = Math.min(Math.floor(4 * Math.pow(n / 100, 0.25)), Math.floor(n / 5), 8);
  }
  ...
  // Build regression: Δy_t = γ*y_{t-1} + Σ(c_i*Δy_{t-i}) + ε_t
  // We'll use lag selection via t-stat significance (simplified AIC-like approach)
  ...
  for (let lag = 1; lag <= maxLag; lag++) {
    // Need enough observations after accounting for lags
    const startIdx = lag;
    const effectiveN = dY.length - startIdx;
    
    if (effectiveN < 10) break;
    
    // Build design matrix
    // Dependent: Δy_t (from index startIdx to end)
    // Regressors: y_{t-1}, Δy_{t-1}, Δy_{t-2}, ..., Δy_{t-lag}
    
    const Y = []; // Δy_t
    const X_y_lag = []; // y_{t-1}
    const X_dy_lags = []; // Lagged differences
    
    for (let t = startIdx; t < dY.length; t++) {
      Y.push(dY[t]);
      X_y_lag.push(series[t]); // y_{t-1} (series is one ahead of dY)
      
      const laggedDiffs = [];
      for (let j = 1; j <= lag; j++) {
        laggedDiffs.push(dY[t - j]);
      }
      X_dy_lags.push(laggedDiffs);
    }
    
    // Multiple regression: Y = γ*X_y_lag + Σ(c_i*X_dy_lags[i]) + ε
    // Using simplified approach: partial out lagged differences first
    
    // For simplicity, we'll use the standard approach with matrix algebra
    // Y = X*β + ε, where X = [y_{t-1}, Δy_{t-1}, ..., Δy_{t-lag}]
    
    const result = multipleRegressionADF(Y, X_y_lag, X_dy_lags);
    
    if (result.valid) {
      // Check if this lag is significant (use last lag's t-stat)
      // If not significant, previous lag was better
      if (lag > 1 && result.lastLagTStat !== null && Math.abs(result.lastLagTStat) < 1.645) {
        break; // Stop adding lags
      }
      
      bestLag = lag;
      bestAdfStat = result.gammaTStat;
      bestGamma = result.gamma;
    }
  }
  
  // Determine p-value category based on critical values
  // For Engle-Granger residuals, use EG critical values
  const cv1 = getEngleGrangerCriticalValue(n, '1%');
  const cv5 = getEngleGrangerCriticalValue(n, '5%');
  const cv10 = getEngleGrangerCriticalValue(n, '10%');
  
  let pValue;
  if (bestAdfStat < cv1) {
    pValue = '<0.01';
  } else if (bestAdfStat < cv5) {
    pValue = '<0.05';
  } else if (bestAdfStat < cv10) {
    pValue = '<0.10';
  } else {
    pValue = '>0.10';
  }
...
/**
 * Multiple regression for ADF test
 * Y = γ*X_level + Σ(c_i*X_lags[i]) + ε
 * 
 * Returns t-statistic for γ (coefficient on lagged level)
 */
function multipleRegressionADF(Y, X_level, X_lags) {
  const n = Y.length;
  const numLags = X_lags[0]?.length || 0;
  const k = 1 + numLags; // Number of regressors (level + lags)
  
  if (n < k + 2) {
    return { valid: false, gamma: 0, gammaTStat: 0, lastLagTStat: null };
  }
  
  // Build full X matrix: [X_level, X_lag1, X_lag2, ...]
  // Using normal equations: β = (X'X)^(-1) X'Y
  
  // For numerical stability, we'll use a simplified approach
  // Since X_level is our main interest, we can use partial regression
  
  // Step 1: Calculate means
  const meanY = Y.reduce((a, b) => a + b, 0) / n;
  const meanXLevel = X_level.reduce((a, b) => a + b, 0) / n;
  const meanXLags = [];
  for (let j = 0; j < numLags; j++) {
    const sum = X_lags.reduce((a, row) => a + row[j], 0);
    meanXLags.push(sum / n);
  }
  
  // Step 2: Build (X'X) matrix and (X'Y) vector
  // X'X is (k x k), X'Y is (k x 1)
  ...
  for (let i = 0; i < n; i++) {
    // Build row of X: [X_level[i], X_lags[i][0], X_lags[i][1], ...]
    const xi = [X_level[i]];
    for (let j = 0; j < numLags; j++) {
      xi.push(X_lags[i][j]);
    }
  ...
  // Step 3: Solve (X'X)β = X'Y using Gaussian elimination
  ...
  const gamma = beta[0]; // Coefficient on lagged level
  
  // Step 4: Calculate residuals and MSE
  let ssRes = 0;
  for (let i = 0; i < n; i++) {
    let predicted = gamma * X_level[i];
    for (let j = 0; j < numLags; j++) {
      predicted += beta[1 + j] * X_lags[i][j];
    }
  ...
  // Step 5: Calculate standard error of gamma
  // SE(γ) = sqrt(MSE * [(X'X)^(-1)]_00)
  ...
  const seGamma = Math.sqrt(mse * XtXInv[0][0]);
  const gammaTStat = seGamma > 0 ? gamma / seGamma : 0;
  
  // Calculate t-stat for last lag (for lag selection)
  let lastLagTStat = null;
  if (numLags > 0) {
    const seLastLag = Math.sqrt(mse * XtXInv[k-1][k-1]);
    lastLagTStat = seLastLag > 0 ? beta[k-1] / seLastLag : 0;
  }
  ...
  return {
    ...
    beta,
    mse,
    rSquared: 1 - ssRes / Y.reduce((sum, y) => sum + (y - meanY)**2, 0)
  };
}
 ...
 * @param {number[]} b - Right-hand side vector (will be modified)
 ...
 * @param {number[][]} A - Matrix to invert
...
/**
 * Proper Engle-Granger Cointegration Test
 * 
 * Step 1: Regress log prices to estimate hedge ratio
 *         log(P_Y) = α + β*log(P_X) + ε
 * 
 * Step 2: Test residuals for stationarity using ADF
 *         Δε_t = γ*ε_{t-1} + Σ(c_i*Δε_{t-i}) + ν_t
 * 
 * If ADF rejects unit root (p-value < 0.05), residuals are stationary
 * and the pair is cointegrated (spread is mean-reverting).
 * 
 * @param {number[]} prices1 - First price series (Y)
 * @param {number[]} prices2 - Second price series (X)
 * @param {string} significanceLevel - '1%', '5%', or '10%' (default: '5%')
 * @returns {{ isCointegrated: boolean, adfStat: number, pValue: string, criticalValue: number, beta: number, alpha: number, halfLife: number, zScore: number, rSquared: number, lags: number }}
 */
function testCointegrationProper(prices1, prices2, significanceLevel = '5%') {
  if (prices1.length !== prices2.length || prices1.length < 20) {
    return {
      isCointegrated: false,
      adfStat: 0,
      pValue: 'insufficient_data',
      criticalValue: -3.37,
      beta: 0,
      alpha: 0,
      halfLife: Infinity,
  ...
  // Step 1: OLS regression on log prices to get beta and residuals
  const logP1 = prices1.map(p => Math.log(p));
  const logP2 = prices2.map(p => Math.log(p));
  
  const regression = olsRegression(logP1, logP2);
  const { alpha, beta, residuals, rSquared } = regression;
  
  // Step 2: Run proper ADF test on residuals
  const adfResult = augmentedDickeyFullerTest(residuals);
  
  // Get critical value for chosen significance level
  const criticalValue = getEngleGrangerCriticalValue(prices1.length, significanceLevel);
  
  // Cointegrated if ADF stat is more negative than critical value
  const isCointegrated = adfResult.adfStat < criticalValue;
  
  // Calculate half-life from gamma (speed of mean reversion)
  // γ represents the adjustment speed: ε_t = (1+γ)*ε_{t-1} + noise
  // Half-life = -ln(2) / ln(1 + γ) when γ < 0
  let halfLife = Infinity;
  if (adfResult.gamma < 0 && adfResult.gamma > -1) {
    const phi = 1 + adfResult.gamma;
    if (phi > 0 && phi < 1) {
  ...
  // Fallback to AR(1) half-life if ADF-based fails
  if (halfLife === Infinity || halfLife > 100) {
    halfLife = calculateHalfLifeAR1(residuals) ?? Infinity;
  }
  ...
  const zScore = stdResidual > 0 ? (currentResidual - meanResidual) / stdResidual : 0;
  
  // Calculate mean reversion rate (for backwards compatibility)
  let meanReversionCount = 0;
  for (let i = 1; i < residuals.length; i++) {
    const prevDev = residuals[i - 1] - meanResidual;
    const currDev = residuals[i] - meanResidual;
    if ((prevDev > 0 && currDev < prevDev) || (prevDev < 0 && currDev > prevDev)) {
      meanReversionCount++;
    }
  }
  const meanReversionRate = meanReversionCount / (residuals.length - 1);
  
  return {
    isCointegrated,
    adfStat: adfResult.adfStat,
    ...
    rSquared,
    lags: adfResult.lags,
    gamma: adfResult.gamma,
    meanReversionRate
  };
}

/**
 * Test cointegration using proper Engle-Granger ADF test
 * 
 * This is the main interface function that maintains backwards compatibility
 * while using the proper statistical test internally.
 * 
 * @param {number[]} prices1 - First price series
 * @param {number[]} prices2 - Second price series
 * @param {number} beta - Hedge ratio (optional, will be recalculated internally using proper OLS on log prices)
 * @returns {{ isCointegrated: boolean, adfStat: number, halfLife: number, meanReversionRate: number, zScore: number, pValue: string, criticalValue: number }}
 */
function testCointegration(prices1, prices2, beta = null) {
  ...
  // Use proper Engle-Granger test
  const result = testCointegrationProper(prices1, prices2, '5%');
  
  // Return with backwards-compatible interface
  return {
    isCointegrated: result.isCointegrated,
    adfStat: result.adfStat,
    halfLife: result.halfLife,
    meanReversionRate: result.meanReversionRate,
    zScore: result.zScore,
    // New fields from proper test
 ...
 * Shows how much spread volatility is suppressed vs directional volatility
 * 
 * @param {number[]} prices1 - Price series for asset 1
 * @param {number[]} prices2 - Price series for asset 2
 * @param {number} beta - Hedge ratio
 * @returns {{ vol1: number, vol2: number, spreadVol: number, volRatio: number, avgDirectionalVol: number }}
 */
function calculateVolatilityMetrics(prices1, prices2, beta) {
  if (prices1.length < 10 || prices2.length < 10 || prices1.length !== prices2.length) {
    return { vol1: null, vol2: null, spreadVol: null, volRatio: null, avgDirectionalVol: null };
  }
  ...
  // Calculate spread: log(P1) - β * log(P2)
  const spreads = prices1.map((p1, i) => Math.log(p1) - beta * Math.log(prices2[i]));
  
  // Calculate spread changes (returns of the spread)
  const spreadReturns = [];
  for (let i = 1; i < spreads.length; i++) {
    spreadReturns.push(spreads[i] - spreads[i - 1]);
  }
  ...
  // Daily volatilities
  const dailyVol1 = stdDev(returns1);
  const dailyVol2 = stdDev(returns2);
  const dailySpreadVol = stdDev(spreadReturns);

  // Annualize (×√365 for daily data)
  const annualizeFactor = Math.sqrt(365);
  const vol1 = dailyVol1 * annualizeFactor * 100; // as percentage
  const vol2 = dailyVol2 * annualizeFactor * 100;
  const spreadVol = dailySpreadVol * annualizeFactor * 100;

  // Average directional volatility
  const avgDirectionalVol = (vol1 + vol2) / 2;

  // Volatility ratio: how much spread vol is vs directional
  // Lower = better beta neutralization
  const volRatio = avgDirectionalVol > 0 ? spreadVol / avgDirectionalVol : null;

  return {
    vol1: parseFloat(vol1.toFixed(2)),
    vol2: parseFloat(vol2.toFixed(2)),
    spreadVol: parseFloat(spreadVol.toFixed(2)),
    avgDirectionalVol: parseFloat(avgDirectionalVol.toFixed(2)),
    volRatio: volRatio !== null ? parseFloat(volRatio.toFixed(3)) : null
  };
}

/**
 * Calculate dynamic entry threshold based on historical Z-score behavior
 * 
 * Analyzes reversion rates at different thresholds and selects optimal entry point
 * that balances trade frequency, reversion reliability, and capital efficiency.
 * 
 * @param {number[]} zScores - Array of historical Z-scores (rolling, from oldest to newest)
 * @param {Object} options - Configuration options
 * @param {number} options.minEvents - Minimum events required for confidence (default: 3)
 * @param {number} options.minRate - Minimum reversion rate required (default: 0.7)
 * @param {number} options.floor - Minimum threshold (default: 2.0)
 * @param {number} options.reversionTarget - Z-score target for "reverted" (default: 0.5)
 * @param {number} options.maxLookforwardDays - Max days to wait for reversion (default: 30)
 * @returns {Object} Dynamic threshold analysis
 */
function calculateDynamicThreshold(zScores, options = {}) {
  const {
    minEvents = 3,
    minRate = 0.7,
    floor = 2.0,
    reversionTarget = 0.5,
    maxLookforwardDays = 30,
    hoursPerCandle = 1  // Assume hourly data by default
  } = options;
  
  const maxLookforward = maxLookforwardDays * 24 / hoursPerCandle;
  const thresholds = [1.5, 2.0, 2.5, 3.0, 3.5];
  const analysis = {};
  ...
  for (const threshold of thresholds) {
    ...
    for (let i = 1; i < zScores.length - maxLookforward; i++) {
      const prevZ = Math.abs(zScores[i - 1]);
      const currZ = Math.abs(zScores[i]);
      
      // Crossed above threshold
      if (prevZ < threshold && currZ >= threshold) {
        const entryZ = currZ;
        let reverted = false;
        let timeToRevert = null;
        let maxAdverseZ = currZ;
        
        // Track if it reverts
        for (let j = 1; j < maxLookforward && i + j < zScores.length; j++) {
          const z = Math.abs(zScores[i + j]);
          maxAdverseZ = Math.max(maxAdverseZ, z);
          
          if (z < reversionTarget) {
            reverted = true;
            timeToRevert = j * hoursPerCandle / 24; // Convert to days
            break;
          }
        }
        
        // Estimate PnL (simplified: entry Z - exit Z, normalized)
        const exitZ = reverted ? Math.abs(zScores[i + Math.floor(timeToRevert * 24 / hoursPerCandle)] || 0) : entryZ;
        const pnl = reverted ? (entryZ - exitZ) / entryZ : -0.5;
        const maxDrawdown = (maxAdverseZ - entryZ) / Math.max(entryZ, 0.1);
        
        events.push({ entryZ, reverted, timeToRevert, maxAdverseZ, pnl, maxDrawdown });
      }
    }
    
    // Calculate statistics
    const n = events.length;
    const revertedCount = events.filter(e => e.reverted).length;
    const rate = n > 0 ? revertedCount / n : 0;
    const avgTime = revertedCount > 0 
      ? events.filter(e => e.reverted).reduce((s, e) => s + e.timeToRevert, 0) / revertedCount 
      : null;
    const avgPnL = n > 0 ? events.reduce((s, e) => s + e.pnl, 0) / n : 0;
    const avgMaxDD = n > 0 ? events.reduce((s, e) => s + e.maxDrawdown, 0) / n : 0;
    
    // Wilson score confidence interval for rate
    let ci95Low = 0, ci95High = 1;
    if (n > 0) {
      const z = 1.96;
      const phat = rate;
      const denom = 1 + z * z / n;
      ...
      ci95Low = Math.max(0, (center - margin) / denom);
      ci95High = Math.min(1, (center + margin) / denom);
    }
    
    // Score = Rate × √Events × (1/AvgTime) × (1 + AvgPnL)
    // Only score if enough events and minimum rate
    const score = (n >= minEvents && rate >= 0.5 && avgTime && avgTime > 0)
      ? rate * Math.sqrt(n) * (1 / Math.max(avgTime, 0.5)) * (1 + avgPnL)
      : 0;
  ...
  // Find best threshold (highest score at or above floor)
  let bestThreshold = floor;
  let bestScore = 0;
  
  for (const t of thresholds) {
    if (t >= floor && analysis[t].score > bestScore && analysis[t].events >= minEvents) {
      bestScore = analysis[t].score;
      bestThreshold = t;
    }
  }
  
  // Fallback: if no threshold at floor meets criteria, check 1.5
  let confidence = 'high';
  let flags = [];
  let recommendation = 'standard_position';
  
  if (bestScore === 0) {
    // Check if 1.5 is viable
    const a15 = analysis[1.5];
    if (a15.events >= 5 && a15.rate >= 0.9) {
      bestThreshold = 1.5;
      bestScore = a15.score;
      confidence = 'low';
      flags.push('tight_spread', 'no_events_above_floor');
      recommendation = 'smaller_position';
    } else {
      // Use default
      bestThreshold = 2.5;
      confidence = 'default';
  ...
  // Check for regime change warning (rate drops significantly at higher thresholds)
  const hasRegimeWarning = 
    (analysis[2.0].rate > 0.7 && analysis[2.5].rate < 0.5 && analysis[2.5].events >= 2) ||
    (analysis[2.5].rate > 0.7 && analysis[3.0].rate < 0.5 && analysis[3.0].events >= 2);
  
  if (hasRegimeWarning) {
    flags.push('regime_change_risk');
  }
  
  // Check for slow reversion
  const bestStats = analysis[bestThreshold];
  if (bestStats.avgTime && bestStats.avgTime > 10) {
    flags.push('slow_reversion');
    recommendation = 'smaller_position';
  }
  ...
  for (let i = 1; i < zScores.length; i++) {
    if ((zScores[i - 1] < 0 && zScores[i] >= 0) || (zScores[i - 1] >= 0 && zScores[i] < 0)) {
      zeroCrossings++;
    }
  }
  const zeroCrossFreq = zeroCrossings / (zScores.length * hoursPerCandle / 24); // per day
  ...
  return {
    ...
    recommendation,
    stats: {
      events: bestStats.events,
      rate: bestStats.rate,
      ratePercent: bestStats.ratePercent,
      avgTime: bestStats.avgTime ? parseFloat(bestStats.avgTime.toFixed(1)) : null,
      avgPnL: parseFloat((bestStats.avgPnL * 100).toFixed(1)),
      ci95: [parseFloat((bestStats.ci95Low * 100).toFixed(0)), parseFloat((bestStats.ci95High * 100).toFixed(0))]
    },
    zeroCrossFreq: parseFloat(zeroCrossFreq.toFixed(2)),
    allThresholds: analysis,
    hasRegimeWarning
  };
}
...
module.exports = {
  ...
  testCointegration,
  testCointegrationProper,  // New: Full Engle-Granger test with proper ADF
  augmentedDickeyFullerTest, // New: Standalone ADF test
  olsRegression,             // New: OLS regression utility
  checkPairFitness,
  calculateVolatilityMetrics,
  calculateHalfLifeAR1,
  calculateDynamicThreshold, // NEW: Dynamic entry threshold
  normalizeSymbol
};
