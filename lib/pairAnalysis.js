 ...
 */
async function analyzeTimeframe(symbol1, symbol2, days, fullOBV1, fullOBV2, obvTimeframes, sharedSdk = null, cutoffTime = null, customZScoreWindow = null) {
  ...
  try {
    ...
    // Use hourly candles for short windows (<= 7 days) to get enough data points for half-life calculation
    // Use daily candles for longer windows
    const useHourly = days <= 7;
    const granularity = useHourly ? '1h' : '1d';
    const hoursNeeded = useHourly ? (days + 2) * 24 : 0; // Extra buffer for hourly
    const daysNeeded = useHourly ? 0 : (days + 5);
    const startTime = useHourly 
      ? endTime - (hoursNeeded * 60 * 60 * 1000)
      : endTime - (daysNeeded * 24 * 60 * 60 * 1000);
    
    const [hl1Data, hl2Data] = await Promise.all([
      sdk.info.getCandleSnapshot(`${symbol1}-PERP`, granularity, startTime, endTime),
      sdk.info.getCandleSnapshot(`${symbol2}-PERP`, granularity, startTime, endTime)
    ]);
    ...
    if (hl1Data?.length > 0 && hl2Data?.length > 0) {
      if (useHourly) {
        // For hourly candles, take the last N hours (where N = days * 24)
        const hoursNeeded = days * 24;
        const sorted1 = hl1Data.sort((a, b) => {
          const t1 = typeof a.t === 'number' ? a.t : new Date(a.t).getTime();
          const t2 = typeof b.t === 'number' ? b.t : new Date(b.t).getTime();
          return t1 - t2;
        });
        const sorted2 = hl2Data.sort((a, b) => {
          const t1 = typeof a.t === 'number' ? a.t : new Date(a.t).getTime();
          const t2 = typeof b.t === 'number' ? b.t : new Date(b.t).getTime();
          return t1 - t2;
        });
        
        // Match by timestamp (within 1 hour tolerance)
        // Use a more efficient approach: create a map for faster lookup
        const matched = [];
        const c2Map = new Map();
        
        // Index c2 by hour (rounded to nearest hour)
        for (const c2 of sorted2) {
          const t2 = typeof c2.t === 'number' ? c2.t : new Date(c2.t).getTime();
          const hourKey = Math.floor(t2 / (60 * 60 * 1000));
          if (!c2Map.has(hourKey)) {
            c2Map.set(hourKey, []);
          }
          c2Map.get(hourKey).push({ time: t2, candle: c2 });
        }
        
        // Match c1 candles with c2 candles
        for (const c1 of sorted1) {
          const t1 = typeof c1.t === 'number' ? c1.t : new Date(c1.t).getTime();
          const hourKey = Math.floor(t1 / (60 * 60 * 1000));
          
          // Check current hour and adjacent hours
          let found = false;
          for (const checkHour of [hourKey - 1, hourKey, hourKey + 1]) {
            const candidates = c2Map.get(checkHour);
            if (candidates) {
              for (const candidate of candidates) {
                const t2 = candidate.time;
                if (Math.abs(t1 - t2) < 60 * 60 * 1000) { // Within 1 hour
                  matched.push({
                    time: t1,
                    price1: parseFloat(c1.c),
                    price2: parseFloat(candidate.candle.c),
                    volume1: parseFloat(c1.v) || 0,
                    volume2: parseFloat(candidate.candle.v) || 0
                  });
                  found = true;
                  break;
                }
        ...
        // Sort by time and take last N hours
        matched.sort((a, b) => a.time - b.time);
        const selected = matched.slice(-hoursNeeded);
        
        if (selected.length >= hoursNeeded * 0.8) { // Allow 20% missing data
          prices1 = selected.map(m => m.price1);
          prices2 = selected.map(m => m.price2);
          volumes1 = selected.map(m => m.volume1);
          volumes2 = selected.map(m => m.volume2);
          currentPrice1 = prices1[prices1.length - 1];
          currentPrice2 = prices2[prices2.length - 1];
        } else {
          throw new Error(`Insufficient hourly data: ${selected.length} hours (need ${hoursNeeded})`);
        }
      } else {
        // Daily candles - original logic
  ...
  // For hourly data, check minimum data points (80% of expected)
  const minDataPoints = days <= 7 ? Math.floor(days * 24 * 0.8) : days;
  if (!prices1 || prices1.length < minDataPoints || !prices2 || prices2.length < minDataPoints) {
    throw new Error(`Insufficient data: ${prices1?.length || 0} points (need ${minDataPoints})`);
  }
  ...
  const defaultConfig = require('../config');
  const zScoreWindow = customZScoreWindow !== null ? customZScoreWindow : defaultConfig.getZScoreWindow();
  ...
  let obvDivergence1 = null, obvDivergence2 = null;
  let obvSignal1 = null, obvSignal2 = null;
  
  // Simple OBV divergence calculation - exactly as your partner specified
  if (obvTimeframes.includes(days) && fullOBV1 && fullOBV2 && 
      fullOBV1.length >= 180 && fullOBV2.length >= 180 && 
      prices1 && prices2 && prices1.length >= days) {
    
    // Get OBV values from 180-day cumulative data
    // Key: fullOBV1[fullOBV1.length - days - 1] = OBV from N days ago
    const obv1Start = fullOBV1[fullOBV1.length - days - 1];  // N days ago
    const obv1End = fullOBV1[fullOBV1.length - 1];           // Today
    const obv2Start = fullOBV2[fullOBV2.length - days - 1];
    const obv2End = fullOBV2[fullOBV2.length - 1];
    ...
    const price1ChangePct = price1Start > 0 
      ? ((price1End - price1Start) / price1Start) * 100 
      : null;
    const price2ChangePct = price2Start > 0 
      ? ((price2End - price2Start) / price2Start) * 100 
      : null;
    ...
    if (Math.abs(obv1Start) > 1) {  // Avoid division by tiny numbers
      obv1ChangePct = ((obv1End - obv1Start) / Math.abs(obv1Start)) * 100;
    }
    if (Math.abs(obv2Start) > 1) {
      obv2ChangePct = ((obv2End - obv2Start) / Math.abs(obv2Start)) * 100;
    }
    
    // Step C: Divergence = OBV% - Price%
    if (obv1ChangePct !== null && price1ChangePct !== null) {
      obvDivergence1 = obv1ChangePct - price1ChangePct;
      
      if (obvDivergence1 >= 30) obvSignal1 = 'STRONG BUY';
      else if (obvDivergence1 >= 10) obvSignal1 = 'BUY';
      else if (obvDivergence1 <= -30) obvSignal1 = 'STRONG SELL';
      else if (obvDivergence1 <= -10) obvSignal1 = 'SELL';
      else obvSignal1 = 'NEUTRAL';
    }
    
    if (obv2ChangePct !== null && price2ChangePct !== null) {
      obvDivergence2 = obv2ChangePct - price2ChangePct;
      
      if (obvDivergence2 >= 30) obvSignal2 = 'STRONG BUY';
      else if (obvDivergence2 >= 10) obvSignal2 = 'BUY';
      else if (obvDivergence2 <= -30) obvSignal2 = 'STRONG SELL';
      else if (obvDivergence2 <= -10) obvSignal2 = 'SELL';
      else obvSignal2 = 'NEUTRAL';
    }
  }
