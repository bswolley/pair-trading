...
// Helper to suppress WebSocket noise from Hyperliquid SDK
...
async function generateTradeReport(tradeKey, trade, isSimulated = false, sharedSdk = null) {
  ...
  try {
    ...
    const hoursBack = Math.min(hoursSinceEntry + 1, 1000);

    // Use shared SDK or create new one
    let sdk = sharedSdk;
    ...
    // Get current prices and historical data from Hyperliquid
    console.log(`Fetching current prices for ${symbol1}/${symbol2}...`);

    const endTime = Date.now();
    const hourlyStartTime = endTime - (hoursBack * 60 * 60 * 1000);
    const dailyStartTime = endTime - (40 * 24 * 60 * 60 * 1000); // 40 days for buffer

    // Fetch hourly and daily data
    console.log(`Fetching hourly data from entry (${hoursSinceEntry} hours ago)...`);
    const [hourly1Data, hourly2Data] = await Promise.all([
      sdk.info.getCandleSnapshot(`${symbol1}-PERP`, '1h', hourlyStartTime, endTime),
      sdk.info.getCandleSnapshot(`${symbol2}-PERP`, '1h', hourlyStartTime, endTime)
    ]);

    console.log('Fetching historical data for current statistics...');
    const [daily1Data, daily2Data] = await Promise.all([
      sdk.info.getCandleSnapshot(`${symbol1}-PERP`, '1d', dailyStartTime, endTime),
      sdk.info.getCandleSnapshot(`${symbol2}-PERP`, '1d', dailyStartTime, endTime)
    ]);

    if (!daily1Data?.length || !daily2Data?.length) {
      throw new Error(`No Hyperliquid data for ${symbol1} or ${symbol2}`);
    }

    // Get current prices from latest candle
    const currentPrice1 = parseFloat(daily1Data[daily1Data.length - 1].c);
    const currentPrice2 = parseFloat(daily2Data[daily2Data.length - 1].c);

    // Build hourly data maps
    const hourly1Map = new Map();
    const hourly2Map = new Map();
    hourly1Data.forEach(c => hourly1Map.set(c.t, { close: parseFloat(c.c), volume: parseFloat(c.v) }));
    hourly2Data.forEach(c => hourly2Map.set(c.t, { close: parseFloat(c.c), volume: parseFloat(c.v) }));

    // Process hourly data into arrays matching Binance format for compatibility
    const commonHourlyTimes = [...hourly1Map.keys()].filter(t => hourly2Map.has(t)).sort((a, b) => a - b);
    const hourly1 = { data: commonHourlyTimes.map(t => [t, 0, 0, 0, hourly1Map.get(t).close, hourly1Map.get(t).volume]) };
    const hourly2 = { data: commonHourlyTimes.map(t => [t, 0, 0, 0, hourly2Map.get(t).close, hourly2Map.get(t).volume]) };

    // Build daily data maps
    const daily1Map = new Map();
    const daily2Map = new Map();
    daily1Data.forEach(c => {
      const date = new Date(c.t).toISOString().split('T')[0];
      daily1Map.set(date, { close: parseFloat(c.c), volume: parseFloat(c.v) });
    });
    daily2Data.forEach(c => {
      const date = new Date(c.t).toISOString().split('T')[0];
      daily2Map.set(date, { close: parseFloat(c.c), volume: parseFloat(c.v) });
    });

    // Get common dates excluding today (incomplete candle)
    const today = new Date().toISOString().split('T')[0];
    const commonDates = [...daily1Map.keys()].filter(d => daily2Map.has(d) && d !== today).sort();
    const selectedDates = commonDates.slice(-34); // Last 34 days

    const prices1 = selectedDates.map(date => daily1Map.get(date).close);
    const prices2 = selectedDates.map(date => daily2Map.get(date).close);
    const volumes1 = selectedDates.map(date => daily1Map.get(date).volume || 0);
    const volumes2 = selectedDates.map(date => daily2Map.get(date).volume || 0);
    ...
    // Calculate half-life using Ornstein-Uhlenbeck model (lag-1 autocorrelation)
    const spreadReturns = [];
    for (let i = 1; i < historicalSpreads.length; i++) {
      spreadReturns.push(historicalSpreads[i] - historicalSpreads[i - 1]);
    }
    const meanSpreadReturn = spreadReturns.reduce((sum, r) => sum + r, 0) / spreadReturns.length;

    // Lag-1 autocorrelation of spread
    let autocorrNum = 0;
    let autocorrDen = 0;
    for (let i = 1; i < spreadReturns.length; i++) {
      autocorrNum += (spreadReturns[i] - meanSpreadReturn) * (spreadReturns[i - 1] - meanSpreadReturn);
      autocorrDen += Math.pow(spreadReturns[i - 1] - meanSpreadReturn, 2);
    }
    const autocorr = autocorrDen > 0 ? autocorrNum / autocorrDen : 0;

    // Half-life formula: -ln(2) / ln(1 + autocorr)
    // Guard against invalid values (autocorr <= -1 would cause ln of non-positive)
    let currentHalfLife = null;
    if (autocorr > -1 && autocorr < 0) {
      currentHalfLife = -Math.log(2) / Math.log(1 + autocorr);
    } else if (autocorr >= 0) {
      currentHalfLife = Infinity; // No mean reversion detected
    }

    // Time to mean reversion: halfLife * log(|z_current| / |z_target|) / log(2)
    const zTarget = 0.5; // Exit threshold
    let timeToMeanReversion = null;
    let halfLivesToExit = null;
    if (currentHalfLife && currentHalfLife !== Infinity && Math.abs(zScore) > zTarget) {
      halfLivesToExit = Math.log(Math.abs(zScore) / zTarget) / Math.log(2);
      timeToMeanReversion = currentHalfLife * halfLivesToExit;
    }
    ...
    // Filter already-fetched daily data to only include candles BEFORE entry date
    const entryDateStr = entryDate.toISOString().split('T')[0];
    const datesBeforeEntry = commonDates.filter(d => d < entryDateStr).sort();
    const entryPrices1 = datesBeforeEntry.map(date => daily1Map.get(date).close);
    const entryPrices2 = datesBeforeEntry.map(date => daily2Map.get(date).close);
    ...
    // Hyperliquid data is already in chronological order
    const hourlyPrices = [];
    for (let i = 0; i < hourly1.data.length; i++) {
      const timestamp = hourly1.data[i][0];
      const candleTime = new Date(timestamp);
      if (candleTime < entryDate) continue;

      hourlyPrices.push({
        timestamp: timestamp,
        price1: parseFloat(hourly1.data[i][4]),
        price2: parseFloat(hourly2.data[i][4])
      });
    }
...
- **Current Half-Life:** ${currentHalfLife === null ? 'N/A' : currentHalfLife === Infinity ? '∞ (no mean reversion)' : `${currentHalfLife.toFixed(2)} days`}
- **Half-Lives to Exit:** ${halfLivesToExit !== null ? halfLivesToExit.toFixed(2) : 'N/A'} ${halfLivesToExit !== null ? `(need |Z| to go from ${Math.abs(zScore).toFixed(2)} → ${zTarget})` : ''}
- **Est. Time to Mean Reversion:** ${timeToMeanReversion !== null ? `${timeToMeanReversion.toFixed(1)} days` : 'N/A'}
- **Time in Trade:** ${daysSinceEntry.toFixed(1)} days
- **Max Duration (2× HL):** ${currentHalfLife !== null && currentHalfLife !== Infinity ? `${(currentHalfLife * 2).toFixed(1)} days` : 'N/A'}
...
async function generateComprehensiveTradeReport() {
  ...
  try {
    ...
    // Create shared Hyperliquid connection
    ...
    for (const tradeKey of tradeKeys) {
      ...
      try {
        await generateTradeReport(tradeKey, trade, isSimulated, sdk);
      } catch (tradeError) {
        console.error(`Failed to process ${tradeKey}: ${tradeError.message}`);
